...
sudo iptables -t mangle -D PREROUTING ! -s $CLIENT_IP ! -d $CLIENT_IP -j MARK --set-mark $MARK_TO_CLIENT > /dev/null 2>&1;
sudo iptables -t mangle -D INPUT ! -s $CLIENT_IP ! -d $CLIENT_IP -j NFQUEUE --queue-num 1 > /dev/null 2>&1;
